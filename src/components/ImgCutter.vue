                  ...
                  <span>
                    <slot name="confirm">
                      <button
                        type="button"
                        class="btn"
                        :class="{ 'btn-primary': drawImg.img, 'btn-error': !drawImg.img}"
                        style="margin-left: 15px"
                        @click="cropPicture(false, !drawImg.img)"
                      >
                        {{ !drawImg.img ? '清空' : '确定' }}
                      </button>
                    </slot>
  ...
  methods: {
    handleOpen: function (img) {
      ...
      let init = (callback) => {
        ...
        _this.$refs["toolBox"].addEventListener(
          "DOMMouseScroll",
          this.scaleImgWheel
        );
        // 判断下窗口高度
        if (_this.isModal === true) {
          ...
          let dialogBoxHeight =
            _this.$refs["dialogMainModalRef"].offsetHeight + 200;
    ...
    cropPicture: function (doNotReset, clear) {
      if(clear) {
        this.$emit('clearImg')
        this.handleClose();
      }
